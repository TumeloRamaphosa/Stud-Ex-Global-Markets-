...
13. [Superpowers Workflow Skill](#13-superpowers-workflow-skill)
14. [DenchClaw Platform Skill](#14-denchclaw-platform-skill)
15. [Claude-Mem Persistent Memory Skill](#15-claude-mem-persistent-memory-skill)
16. [MCP Token Optimization Skill](#16-mcp-token-optimization-skill)
17. [YouTube Transcript & Analysis Skill](#17-youtube-transcript--analysis-skill)
18. [Deapi AI Skills Marketplace](#18-deapi-ai-skills-marketplace)
...
## 13. Superpowers Workflow Skill

**Status:** Installed
**Source:** [obra/superpowers](https://github.com/obra/superpowers)
**Category:** Development Workflow & TDD
**Install Path:** `.claude/skills/superpowers/`

### Overview
A complete software development workflow system with 14 composable skills. Enforces professional-grade engineering discipline with strict TDD, structured planning, sub-agent coordination, and automated code review.

### Capabilities
- **Brainstorming** — Refine ideas through clarifying questions before any code is written
- **Planning** — Break work into small 2-5 min tasks with exact file paths and verification steps
- **Test-Driven Development** — Strict RED-GREEN-REFACTOR cycles; no production code without a failing test
- **Subagent-Driven Development** — Dispatch parallel sub-agents per task with two-stage review
- **Systematic Debugging** — Root cause analysis with evidence-based verification
- **Code Review** — Automated review against the plan with severity-based reporting
- **Git Worktrees** — Isolated branches for safe experimentation
- **Branch Completion** — Merge/PR workflows with cleanup
...
| `/execute-plan` | Execute the plan systematically |
| `superpowers:test-driven-development` | Enforce strict TDD |
| `superpowers:systematic-debugging` | Root cause analysis workflow |
| `superpowers:requesting-code-review` | Automated code review |
| `superpowers:dispatching-parallel-agents` | Spin up sub-agents per task |
| `superpowers:finishing-a-development-branch` | Verify, merge, cleanup |
| `superpowers:using-git-worktrees` | Create isolated workspaces |

### Key Files
```
.claude/skills/superpowers/skills/        — All 14 skill definitions
.claude/skills/superpowers/commands/      — Legacy slash commands (deprecated)
```

---

## 14. DenchClaw Platform Skill

**Status:** Installed
**Source:** [DenchHQ/DenchClaw](https://github.com/DenchHQ/DenchClaw)
**Category:** AI CRM, App Builder & Dev Workflow
**Install Path:** `.claude/skills/denchclaw/`

### Overview
A locally-hosted AI CRM and productivity platform running at `localhost:3100`. Provides database management with DuckDB, a sandboxed app builder, browser automation, and a structured sprint-based development workflow (GStack) with 18 AI specialist roles.

### What is DuckDB?
DuckDB is an in-process SQL database (like SQLite but optimized for analytics). Blazing fast for queries on structured data — perfect for CRM objects, deal pipelines, and market analytics. No server needed, runs embedded.

### Capabilities
- **CRM** — CRUD operations on contacts, companies, deals via DuckDB; kanban, calendar, timeline, gallery views
- **App Builder** — Create sandboxed web apps with React/TSX; bridge API for data, AI, webhooks, cron jobs
- **Browser Automation** — Chromium browser with existing auth state for web scraping and automation
- **GStack Sprint Workflow** — Think → Plan → Build → Review → Test → Ship → Reflect with 18 roles (CEO, Eng Manager, Designer, QA Lead, SRE, Release Engineer, etc.)

### How to Invoke
| Command | What It Does |
|---------|-------------|
| `denchclaw:crm` | Manage contacts, deals, companies via DuckDB |
| `denchclaw:app-builder` | Build sandboxed web apps |
| `denchclaw:browser` | Automate browser tasks with your auth state |
| `denchclaw:gstack` | Sprint-based dev workflow with AI specialist roles |

### Running the UI
```bash
npx denchclaw@latest    # Requires Node 22+
# Opens web UI at http://localhost:3100
```

### Use Cases for Studex
1. **CRM for deal management** — Track investors, entrepreneurs, and deals
2. **Internal tool builder** — Rapid prototyping of dashboards and admin tools
3. **Market data scraping** — Browser automation for external data sources
4. **CTO workflow** — GStack organizes AI as a virtual dev team you oversee

### Key Files
```
.claude/skills/denchclaw/skills/crm/         — CRM skill definitions
.claude/skills/denchclaw/skills/app-builder/ — App builder skills
.claude/skills/denchclaw/skills/browser/     — Browser automation
.claude/skills/denchclaw/skills/gstack/      — Sprint workflow skills
```

---

## 15. Claude-Mem Persistent Memory Skill

**Status:** Installed
**Source:** [thedotmack/claude-mem](https://github.com/thedotmack/claude-mem)
**Category:** Session Memory & Context Persistence
**Install Path:** `.claude/skills/claude-mem/`

### Overview
Persistent memory system for Claude Code that automatically captures context from coding sessions, compresses and stores it, and re-injects relevant context into future sessions. Solves the "amnesia problem" — Claude remembers past decisions, debugging sessions, and architectural choices.

### Capabilities
- **Automatic Context Capture** — 5 lifecycle hooks (SessionStart, UserPromptSubmit, PostToolUse, Stop, SessionEnd)
- **SQLite + Chroma Vector DB** — Persistent local storage with hybrid semantic + keyword search
- **Progressive Disclosure** — 3-layer retrieval (Search → Timeline → Get_observations) with ~10x token savings
- **Web UI** — Real-time memory stream viewer at `http://localhost:37777`
- **Privacy Controls** — Content in `<private>` tags excluded from storage

### How to Invoke
| Command | What It Does |
|---------|-------------|
| `/mem-search` | Query past session context with natural language |
| `claude-mem:do` | Execute memory-aware actions |
| `claude-mem:make-plan` | Create plans with historical context |
| `claude-mem:smart-explore` | Explore codebase with memory-augmented search |
| `claude-mem:timeline-report` | Generate chronological activity reports |

### Data Storage
```
~/.claude-mem/settings.json    — Configuration (auto-generated)
~/.claude-mem/                 — SQLite database, sessions, observations
Chroma vector DB               — Semantic search index
```

### Exporting Data
The SQLite database can be exported to JSON/CSV for upload to Google NotebookLM:
```bash
sqlite3 ~/.claude-mem/memory.db ".mode csv" ".headers on" ".output memory_export.csv" "SELECT * FROM observations;"
```

### Key Files
```
.claude/skills/claude-mem/plugin/skills/     — Skill definitions
.claude/skills/claude-mem/plugin/hooks/      — Lifecycle hooks
.claude/skills/claude-mem/plugin/modes/      — Custom modes
```

---

## 16. MCP Token Optimization Skill

**Status:** Installed
**Source:** Custom skill based on [Eddy Says Hi podcast](https://www.youtube.com/watch?v=BK169j7wQas)
**Category:** Performance & Cost Optimization
**Install Path:** `.claude/skills/mcp-token-optimization/`

### Overview
Strategies for reducing MCP (Model Context Protocol) token bloat to minimize costs, improve response speed, and manage context windows efficiently.

### Capabilities
- Tool description minimization
- Batch operation patterns
- Response filtering and pagination
- Context window management best practices
- Skill-specific optimization (progressive disclosure)
- MCP server configuration tuning

### How to Invoke
Reference `mcp-token-optimization` when sessions run slow or token costs are high.

### Quick Checklist
- Are all enabled MCP servers actually needed?
- Using dedicated tools (Read, Grep, Glob) instead of Bash?
- Requesting specific data rather than broad queries?
- Delegating research to sub-agents to protect main context?
- Using progressive disclosure for memory/search results?

---

## 17. YouTube Transcript & Analysis Skill

**Status:** Installed
**Source:** [jftuga/transcript-critic](https://github.com/jftuga/transcript-critic)
**Category:** Content Analysis & Transcription
**Install Path:** `.claude/skills/transcript-critic/`

### Overview
Transcribes audio/video content (including YouTube) using whisper.cpp, then produces structured critical analysis. Uses yt-dlp + ffmpeg + whisper.cpp. Supports many video platforms beyond YouTube.

### Capabilities
- YouTube video transcription (subtitles first, Whisper fallback)
- Multi-platform support via yt-dlp (Vimeo, Twitch, etc.)
- Structured critical analysis of transcribed content
- Local processing — no external API keys required for transcription

### How to Invoke
| Command | What It Does |
|---------|-------------|
| `/transcribe <URL>` | Transcribe a YouTube video or audio file |
...
### Key Files
```
.claude/skills/transcript-critic/SKILL.md           — Skill definition
.claude/skills/transcript-critic/transcribe.sh       — Transcription script
.claude/skills/transcript-critic/ANALYSIS_PROMPT.md  — Analysis template
```

---

## 18. Deapi AI Skills Marketplace

**Status:** Installed
**Source:** [deapi-ai/claude-code-skills](https://github.com/deapi-ai/claude-code-skills)
**Category:** Multi-Purpose AI Skills Collection
**Install Path:** `.claude/skills/deapi-skills/`

### Overview
A curated skills marketplace including YouTube/audio transcription, image generation, text-to-speech, OCR, and more. Provides additional AI capabilities beyond the core Claude Code toolset.

### Key Files
```
.claude/skills/deapi-skills/deapi/SKILL.md     — Main skill definition
.claude/skills/deapi-skills/deapi/commands/     — Available commands
```
