...
export default function MarketAuditPage() {
  ...
  const runAudit = async () => {
    ...
    try {
      const res = await fetch('/api/market-audit', {
        method: 'POST',
  ...
  const downloadPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <p class="footer">Generated by Studex Global Markets AI Marketing Audit Engine</p>
      </body>
      </html>
    `);
          ...
          <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h1 className="text-3xl sm:text-4xl font-bold">Market Audit</h1>
                  <p className="text-gray-400 text-sm">AI-powered website analysis &amp; marketing intelligence</p>
                </div>
              </div>
            </div>
                      ...
                      <div className="flex gap-2">
                        {['critical', 'high', 'medium'].map((sev) => {
                          ...
                          return (
                            <Badge
                              key={sev}
                              variant={sev === 'critical' ? 'error' : sev === 'high' ? 'warning' : 'info'}
                              size="sm"
                            >
                              {count} {sev}
                            </Badge>
                  ...
                  <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-4">
                      {['good', 'needs-fix', 'missing'].map((status) => {
                        ...
                        const config: Record<string, { label: string; color: string; variant: 'success' | 'warning' | 'error' }> = {
                          'good': { label: 'Passing', color: 'text-green-400', variant: 'success' },
                          'needs-fix': { label: 'Needs Fix', color: 'text-amber-400', variant: 'warning' },
                          'missing': { label: 'Missing', color: 'text-red-400', variant: 'error' },
                        };
